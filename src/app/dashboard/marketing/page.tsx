...
export default function MarketingPage() {
  ...
  const loadFilterPreview = async () => {
    ...
    try {
      ...
      // Ensure days has a default value for API requests
      const params = { ...filterParams };
      if ((selectedFilter === 'expiring_soon' || selectedFilter === 'new_clients') && !params.days) {
        params.days = 7;
      }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between mb-6">
          ...
          <div className="flex gap-3 sm:hidden">
            ...
            <button
              ...
              className="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors flex items-center gap-2 whitespace-nowrap"
            >
          ...
          <button
            ...
            className="hidden sm:flex px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors items-center gap-2"
          >
            <Plus className="w-4 h-4" />
            Создать сообщение
          </button>
                  ...
                  <div className="flex flex-col sm:hidden">
                    ...
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-400 text-sm mb-2">{formatDate(message.createdAt)}</p>
      ...
      {showSendModal && sendingMessage && (
        <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-[110]">
          <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-lg max-h-[90vh] overflow-hidden shadow-xl flex flex-col">
            {/* Header - compact */}
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
              <h2 className="text-lg font-semibold text-white">Отправить сообщение</h2>
              <button
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="bg-gray-700/50 border border-gray-600 rounded-lg p-4">
                    ...
                    <h4 className="text-white font-medium mb-2">{sendingMessage.title}</h4>
                    <p className="text-gray-300 text-sm leading-relaxed">{sendingMessage.content}</p>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <label className={`flex items-center p-3 border rounded-lg cursor-pointer transition-colors ${
                      selectedFilter === 'all' 
                        ...
                        : 'border-gray-600 bg-gray-700/50 hover:border-blue-500/50'
                    }`}>
                      <input
                        type="radio"
                        name="filter"
                      ...
                      <div>
                        <div className="text-white font-medium">Все клиенты</div>
                        <div className="text-gray-400 text-sm">Клиенты с Telegram ID</div>
                      </div>
                    </label>
                    ...
                    <label className={`flex items-center p-3 border rounded-lg cursor-pointer transition-colors ${
                      selectedFilter === 'expiring_soon' 
                        ...
                        : 'border-gray-600 bg-gray-700/50 hover:border-orange-500/50'
                    }`}>
                      <input
                        type="radio"
                        name="filter"
                      ...
                      <div className="flex-1">
                        <div className="text-white font-medium">Заканчивающиеся</div>
                        <div className="text-gray-400 text-sm">Абонементы истекают скоро</div>
                        {selectedFilter === 'expiring_soon' && (
                          <div className="flex items-center gap-2 mt-2">
                            <input
                              type="number"
                              placeholder="7"
                              value={filterParams.days || ''}
                              onChange={(e) => setFilterParams({ ...filterParams, days: e.target.value ? parseInt(e.target.value) : undefined })}
                    ...
                    <label className={`flex items-center p-3 border rounded-lg cursor-pointer transition-colors ${
                      ...
                      <div className="flex-1">
                        <div className="text-white font-medium">Новые клиенты</div>
                        <div className="text-gray-400 text-sm">Зарегистрированные недавно</div>
                        {selectedFilter === 'new_clients' && (
                          <div className="flex items-center gap-2 mt-2">
                            <input
                              type="number"
                              ...
                              max="30"
                            />
                            <span className="text-gray-400 text-sm">дней назад</span>
                          </div>
                        )}
                      </div>
                    </label>
                    ...
                    <label className={`flex items-center p-3 border rounded-lg cursor-pointer transition-colors ${
                      ...
                      <div className="flex-1">
                        <div className="text-white font-medium">По тарифу</div>
                        <div className="text-gray-400 text-sm">Клиенты с определенным тарифом</div>
                        {selectedFilter === 'by_tariff' && (
                          <select
                            value={filterParams.tariffId || ''}
                            onChange={(e) => setFilterParams({ ...filterParams, tariffId: e.target.value })}
                            className="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded text-white text-sm mt-2"
                          >
                  ...
                  <div className="bg-gray-700/50 border border-gray-600 rounded-lg p-4">
                    ...
                    {previewLoading ? (
                      ...
                      <div className="flex items-center justify-between p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                        <span className="text-blue-400 font-medium">{filterPreview.count} получателей</span>
                        {filterPreview.count > 0 && (
                          <button
                            onClick={() => setShowClientsList(!showClientsList)}
                            className="text-blue-400 hover:text-blue-300 text-sm"
                          >
                            {showClientsList ? 'Скрыть' : 'Показать'}
                          </button>
                        )}
                      </div>
                    ) : (
                    ...
                    {showClientsList && filterPreview && filterPreview.clients.length > 0 && (
                      <div className="mt-3 max-h-32 overflow-y-auto bg-gray-600/50 rounded p-2">
                        {filterPreview.clients.map((client, index) => (
                          <div key={client.id} className="flex justify-between text-sm py-1">
                            <span className="text-gray-300">{index + 1}. {client.fullName}</span>
      ...
      {showResultsModal && selectedMessageForResults && (
        <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-[130]">
          <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-2xl max-h-[85vh] overflow-hidden shadow-xl">
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
                  ...
                  {sendResults.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {sendResults.map((result) => {
                        ...
                        return (
                          <div key={result.id} className="bg-gray-700/50 border border-gray-600 rounded-lg p-3">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-white font-medium">{result.totalRecipients}</span>
                            </div>
                            <div className="flex items-center justify-between text-xs">
                              <span className="text-gray-400">{formatDate(result.sentAt)}</span>
                              <div className="flex items-center gap-3">
                                <span className="text-green-400">✓ {result.successCount}</span>
                                <span className="text-red-400">✗ {result.failedCount}</span>
                                <span className="text-blue-400">⏳ {result.pendingCount}</span>
                              </div>
                            </div>
                          </div>
                        );
